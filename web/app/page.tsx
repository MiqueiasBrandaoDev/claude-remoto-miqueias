...
export default function Page() {
  ...
  const [client, setClient] = React.useState("miqueias");
  ...
  // lembra o ultimo cliente selecionado (restaura no reload)
  React.useEffect(() => {
    const c = localStorage.getItem("camozzi:lastClient");
    if (c) setClient(c);
  }, []);

  // salva a conversa ativa por cliente (pra restaurar no reload)
  React.useEffect(() => {
    if (activeSession) localStorage.setItem("camozzi:lastSession:" + client, activeSession);
  }, [activeSession, client]);

  // restaura a conversa ativa daquele cliente quando o worker fica online
  const restoredRef = React.useRef<string | null>(null);
  React.useEffect(() => {
    if (!online) return;
    if (restoredRef.current === client) return; // ja restaurou esse cliente nesta sessao
    restoredRef.current = client;
  ...
  const changeClient = (c: string) => {
    setClient(c);
    localStorage.setItem("camozzi:lastClient", c);
    setMessages([]);
    setActiveSession(null);
    closeFile();
    setTree([]);
    if (explorerOpen) loadTree(c);
  };
  ...
  const submit = (text: string) => {
    ...
    if (t === "/config") { setSettingsOpen(true); return; }
    if (t === "/memory") { if (!explorerOpen) setExplorerOpen(true); openFile("CLAUDE.md"); return; }
    if (t === "/help") {
    ...
    if (t === "/init") { runClaude("Analise o workspace e crie ou atualize o CLAUDE.md com o contexto da empresa (nome, área, tom de voz, o que você deve fazer).", "/init"); return; }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* 3 paineis redimensionaveis: explorer | documento | chat (render condicional = robusto) */}
        ...
        <AnimatePresence>
          {!online && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 z-[60] bg-background/95 backdrop-blur-sm flex items-center justify-center"
            >
        ...
        <AnimatePresence>
          {historyOpen && (
            <>
              ...
              />
              <motion.div
                className="absolute top-0 right-0 h-full w-[340px] bg-panel border-l border-border z-50 flex flex-col"
                initial={{ x: 360 }}
                animate={{ x: 0 }}
                exit={{ x: 360 }}
                  ...
                  {historySessions === null ? (
                    ...
                    historySessions.map((s) => (
                      <button
                        ...
                        className={cn(
                          "w-full text-left p-2.5 rounded-lg mb-1 border border-transparent hover:bg-panel-2 hover:border-border transition-colors",
            ...
            <>
              ...
              >
                <div className="px-4 h-14 flex items-center border-b border-border shrink-0">
                  <span className="text-sm font-semibold">Preferencias</span>
                </div>
                <div className="p-4 space-y-4 text-sm">
                  <div>
                    <div className="text-xs text-muted-foreground mb-1">Usuário</div>
                    <div className="bg-panel-2 border border-border rounded-lg px-3 py-2">{client}</div>
                  </div>
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Cada usuário tem o próprio histórico. Os arquivos são compartilhados por toda a Camozzi.
                  </p>
                </div>
              </motion.div>
            </>
